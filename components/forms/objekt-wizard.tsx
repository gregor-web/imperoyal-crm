    ...
    <div className="min-h-[600px] flex flex-col">
      ...
      <div className="mb-8">
              ...
              <button
                ...
                className={`
                  ...
                  ${currentStep === step.id
                    ...
                    : currentStep > step.id
                    ? 'bg-[#34C759]/080 text-white cursor-pointer hover:bg-green-600'
                    : 'bg-[#253546] text-[#7A9BBD]'
                  }
                `}
              >
              ...
              {index < STEPS.length - 1 && (
                <div
                  className={`w-4 sm:w-8 h-1 mx-1 rounded ${
                    currentStep > step.id ? 'bg-[#34C759]/080' : 'bg-[#253546]'
                  }`}
                />
              )}
            </div>
        ...
        <div className="text-center">
          <h2 className="text-xl font-bold text-[#EDF1F5]">{STEPS[currentStep - 1].title}</h2>
          <p className="text-[#7A9BBD] text-sm">{STEPS[currentStep - 1].description}</p>
          {strasse && plz && ort && currentStep > 1 && (
            <p className="text-xs text-[#7A9BBD] mt-1">{strasse}, {plz} {ort}</p>
          )}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onFormSubmit)} className="flex-1 flex flex-col">
        <div className="flex-1">
          ...
          {currentStep === 2 && (
            <div className="space-y-4">
              ...
              <div className="bg-[#162636] border border-white/[0.12] rounded-lg p-4">
                <h4 className="text-sm font-semibold text-[#EDF1F5] mb-3">Anzahl Einheiten (bestimmt Felder in Step 9)</h4>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  ...
                  />
                  <div>
                    <label className="block text-sm font-medium text-[#EDF1F5] mb-1">Stellplätze</label>
                    <input
                      type="number"
                      value={stellplaetze}
                      onChange={(e) => setStellplaetze(Number(e.target.value) || 0)}
                      placeholder="0"
                      className="w-full px-3 py-2 bg-white/50 backdrop-blur-sm border border-white/[0.08] rounded-lg focus:ring-2 focus:ring-[#7A9BBD] focus:border-[#5B7A9D]"
                    />
                  </div>
                </div>
                <p className="text-xs text-[#7A9BBD] mt-2">
          ...
          {currentStep === 6 && (
            <div className="space-y-4">
              ...
              />
              <p className="text-sm text-[#7A9BBD] bg-[#162636] p-3 rounded-lg">
          ...
          {currentStep === 9 && (
            <div className="space-y-4">
              {einheiten.length === 0 ? (
                <div className="text-center py-8 bg-[#162636] rounded-lg">
                  <p className="text-[#7A9BBD]">
                ...
                <>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-4">
                    <h3 className="text-base sm:text-lg font-semibold text-[#EDF1F5]">
                        ...
                        <div key={index} className={`border rounded-lg p-3 bg-${nutzungColor}-50/50 border-${nutzungColor}-200`}>
                          ...
                          <div className="flex items-center justify-between">
                            <button
                              ...
                              className="flex items-center gap-2 text-sm font-medium text-[#EDF1F5] hover:text-[#EDF1F5]"
                            >
                              ...
                              {einheit.flaeche && einheit.kaltmiete && (
                                <span className="text-[#7A9BBD] font-normal">
                          ...
                          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-3">
                            <div>
                              <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Nutzung</label>
                              <select
                                value={einheit.nutzung}
                                onChange={(e) => updateEinheit(index, 'nutzung', e.target.value as 'Wohnen' | 'Gewerbe' | 'Stellplatz')}
                                className="w-full px-2 py-1.5 text-sm border border-white/[0.08] rounded-lg"
                              >
                            ...
                            <div>
                              <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Fläche (m²)</label>
                              <input
                                type="number"
                                step="0.01"
                            ...
                            <div>
                              <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Kaltmiete (€)</label>
                              <input
                                type="number"
                                step="0.01"
                            ...
                            <div>
                              <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Vertragsart</label>
                              <select
                          ...
                          {isExpanded && (
                            <>
                              ...
                              <div className="mt-4 pt-3 border-t border-white/[0.08]">
                                <h5 className="text-xs font-semibold text-[#7A9BBD] mb-2">Vertragsdaten</h5>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Vertragsbeginn</label>
                                    <input
                                      type="date"
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Marktmiete (€/m²)</label>
                                    <input
                                      type="number"
                                      step="0.01"
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Letzte Erhöhung</label>
                                    <input
                                      type="date"
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Höhe (€)</label>
                                    <input
                                      type="number"
                                      step="0.01"
                              ...
                              <div className="mt-3 pt-3 border-t border-white/[0.08]">
                                <h5 className="text-xs font-semibold text-[#7A9BBD] mb-2">§558 BGB - Vergleichsmiete</h5>
                                <div className="grid grid-cols-2 gap-3">
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Datum §558</label>
                                    <input
                                      type="date"
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Höhe §558 (€)</label>
                                    <input
                                      type="number"
                                      step="0.01"
                              ...
                              {/* §559 BGB */}
                              <div className="mt-3 pt-3 border-t border-white/[0.08]">
                                <h5 className="text-xs font-semibold text-[#7A9BBD] mb-2">§559 BGB - Modernisierung</h5>
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Datum §559</label>
                                    <input
                                      type="date"
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Art</label>
                                    <input
                                      type="text"
                                  ...
                                  <div>
                                    <label className="block text-xs font-medium text-[#6B8AAD] mb-1">Höhe (€/Mon)</label>
                                    <input
                                      type="number"
                                      step="0.01"
        ...
        {/* Navigation */}
        <div className="flex justify-between items-center pt-4 sm:pt-6 mt-4 sm:mt-6 border-t border-white/[0.08] gap-2">
          ...
          <div className="text-xs sm:text-sm text-[#7A9BBD] text-center">
