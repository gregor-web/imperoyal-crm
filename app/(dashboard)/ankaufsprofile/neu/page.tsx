    ...
    switch (currentStep) {
        ...
        return (
          <div className="space-y-6">
            <div className="text-center mb-6 sm:mb-8">
              <h2 className="text-xl font-semibold text-[#EDF1F5]">Wie soll Ihr Ankaufsprofil heißen?</h2>
              <p className="text-[#6B8AAD] mt-2">Geben Sie einen aussagekräftigen Namen für Ihr Profil ein</p>
            </div>
      ...
      case 2:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6 sm:mb-8">
              <h2 className="text-xl font-semibold text-[#EDF1F5]">Welches Investitionsvolumen?</h2>
              <p className="text-[#6B8AAD] mt-2">Definieren Sie Ihre Preisspanne für Ankäufe</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
              <div>
                ...
                />
                <p className="text-xs text-[#7A9BBD] mt-1">
            ...
            <div>
              <label className="block text-sm font-medium text-[#EDF1F5] mb-2">Schnellauswahl</label>
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6 sm:mb-8">
              <h2 className="text-xl font-semibold text-[#EDF1F5]">Welche Assetklassen?</h2>
              <p className="text-[#6B8AAD] mt-2">Wählen Sie alle relevanten Immobilientypen aus</p>
            </div>
                ...
                return (
                  <button
                    ...
                    className={`p-4 rounded-xl text-sm font-medium transition-all border-2 ${
                      isSelected
                        ? 'bg-[#7A9BBD]/15 border-[#5B7A9D] text-[#6B8AAD]'
                        : 'bg-[#1E2A3A] border-white/[0.08] text-[#EDF1F5] hover:border-white/[0.12] hover:bg-[#162636]'
                    }`}
                  >
            ...
            <p className="text-center text-sm text-[#7A9BBD]">
      ...
      case 4:
        return (
          <div className="space-y-6">
            <div className="text-center mb-6 sm:mb-8">
              <h2 className="text-xl font-semibold text-[#EDF1F5]">Standort & Renditeerwartung</h2>
              <p className="text-[#6B8AAD] mt-2">Definieren Sie Ihre geografischen und finanziellen Kriterien</p>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-[#EDF1F5] mb-1">
              ...
              <textarea
                ...
                rows={3}
                className="glass-input w-full px-4 py-2.5 rounded-lg text-[#EDF1F5] placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Weitere Kriterien oder Anmerkungen..."
              />
            </div>

            {/* Summary */}
            <div className="mt-8 p-4 bg-[#162636] rounded-xl">
              <h3 className="text-sm font-semibold text-[#EDF1F5] mb-3">Zusammenfassung</h3>
              <div className="space-y-2 text-sm">
                <div className="flex flex-wrap justify-between gap-1">
                  <span className="text-[#6B8AAD]">Profilname:</span>
                  <span className="font-medium text-[#EDF1F5] truncate max-w-[180px] sm:max-w-[250px]">{formData.name}</span>
                </div>
                <div className="flex flex-wrap justify-between gap-1">
                  <span className="text-[#6B8AAD] flex-shrink-0">Volumen:</span>
                  <span className="font-medium text-[#EDF1F5] text-right">
                ...
                <div className="flex justify-between">
                  <span className="text-[#6B8AAD]">Assetklassen:</span>
                  <span className="font-medium text-[#EDF1F5]">{formData.assetklassen.length} ausgewählt</span>
                </div>
                {formData.regionen && (
                  <div className="flex justify-between">
                    <span className="text-[#6B8AAD]">Regionen:</span>
                    <span className="font-medium text-[#EDF1F5] truncate max-w-[140px] sm:max-w-[200px]">{formData.regionen}</span>
                  </div>
                )}
                {formData.rendite_min && (
                  <div className="flex justify-between">
                    <span className="text-[#6B8AAD]">Mindestrendite:</span>
                    <span className="font-medium text-[#EDF1F5]">{formData.rendite_min}%</span>
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link href="/ankaufsprofile" className="p-2 hover:bg-[#162636] rounded-lg">
          <ArrowLeft className="w-5 h-5 text-[#7A9BBD]" />
        </Link>
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-[#EDF1F5]">Neues Ankaufsprofil</h1>
          <p className="text-[#6B8AAD] mt-1">Schritt {currentStep} von {STEPS.length}</p>
        </div>
      </div>
            ...
            return (
              <div key={step.id} className="flex items-center flex-shrink-0">
                <div className="flex flex-col items-center">
                  ...
                  <span className={`text-[10px] sm:text-xs mt-1 max-w-[60px] sm:max-w-none text-center truncate ${isCurrent ? 'text-[#7A9BBD] font-medium' : 'text-[#7A9BBD]'}`}>
                    {step.title}
                  </span>
                </div>
                {index < STEPS.length - 1 && (
                  <div
                    className={`h-1 mx-1 sm:mx-2 rounded flex-shrink-0 ${
                      currentStep > step.id ? 'bg-[#5B7A9D]/40' : 'bg-[#253546]'
                    }`}
