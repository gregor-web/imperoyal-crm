      ...
      try {
        ...
        await fetch(webhookUrl, {
          ...
          body: JSON.stringify({
            actionId: 3,
            to: data.email,
            subject: `Neues Onboarding: ${data.name} – ${data.objekte.length} Objekt(e)`,
            html: `<h2>Neues Onboarding eingegangen</h2>
<p><strong>Mandant:</strong> ${data.name}</p>
<p><strong>Ansprechpartner:</strong> ${ansprechpartner}</p>
<p><strong>E-Mail:</strong> ${data.email}</p>
<p><strong>Telefon:</strong> ${data.telefon || '–'}</p>
<hr/>
<p><strong>Objekte:</strong> ${data.objekte.length}</p>
<p><strong>Einheiten gesamt:</strong> ${totalEinheiten}</p>
<p><strong>Gesamtvolumen:</strong> ${totalKaufpreis.toLocaleString('de-DE')} €</p>
<p><strong>Adressen:</strong> ${data.objekte.map(o => `${o.strasse}, ${o.plz} ${o.ort}`).join(' | ')}</p>`,
          }),
        }).then(res => {
          if (!res.ok) {
